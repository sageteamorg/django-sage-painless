...
class ReportUserAnswer:
    ...
    def init_report_file(self):
        ...
        self.create_dir_is_not_exists('docs')
        self.create_file_is_not_exists(
            f'{settings.BASE_DIR}/{self.app_label}/docs/{self.file_prefix}-generation-report.json'
        )

    def add_question_answer(self, question, answer):
        """append question answer to report file"""
        with open(f'{settings.BASE_DIR}/{self.app_label}/docs/{self.file_prefix}-generation-report.json', 'r+') as file:
            try:
                data = json.load(file)
            except:
                data = {}
